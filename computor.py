...
class PolynomialSolver:
    def __init__(self):
        self.coefficients = {}  # Dictionary to store coefficients by degree
        self.max_degree = 0     # Maximum degree of the polynomial
    ...
    def parse_equation(self, equation):
        ...
        # Parse both sides
        left_coefficients = self.parse_expression(left_side)
        right_coefficients = self.parse_expression(right_side)
        ...
        self.coefficients = left_coefficients
    ...
    def get_reduced_form(self):
        ...
        for power in sorted(self.coefficients.keys()):
            coef = self.coefficients.get(power, 0)
    ...
    def solve(self):
        ...
        if degree == 0:
            ...
            result.append(self.as_fraction(solution))
        elif degree == 2:
            ...
            if abs(discriminant) < 1e-10:  # discriminant ≈ 0
                ...
                result.append(self.as_fraction(solution1))
                result.append(self.as_fraction(solution2))
            else:  # discriminant < 0
                ...
                result.append(f"{self.as_fraction(real_part)} + {self.as_fraction(imag_part)}i")
                result.append(f"{self.as_fraction(real_part)} - {self.as_fraction(imag_part)}i")
...
def main():
    ...
    2. Creating the solver object
    3. Parsing and solving the equation
    4. Displaying the results
    5. Error handling
    
    Example usage: ./computor.py "5 * X^2 + 4 * X - 3 = 0"
    """
    ...
    try:
        ...
        # Display results as required by the subject
        print(f"Reduced form: {solver.get_reduced_form()}")
        print(solver.solve())
        
    except Exception as e:
        # Handle errors gracefully (bonus - error management)
        print(f"Error: {e}")
        sys.exit(1)
